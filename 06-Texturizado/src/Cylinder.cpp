...
Cylinder::Cylinder(int slices, int stacks, float topRadius, float bottomRadius, float height) {
	...
	for (int i = 0; i <= stacks; i++) {
		...
		float U = i / (float)stacks;//se agrego para las texturas
		for (int j = 0; j <= slices; j++) {
			float V = j / (float)slices;//se agrego para las texturas
	...
	for (int i = slices; i >= 0; i--) {
		...
		float U = cos(i * dTheta) * 0.5 + 0.5;//se modifico esta linea para agregar texturas
		float V = sin(i * dTheta)*0.5 + 0.5;//se modifico esta linea para agregar texturas
		vertexArray[count++] = Vertex(glm::vec3(x, y, z), glm::vec2(U,V), glm::vec3(0, 1, 0));//se modifico esta linea para agregar texturas
	}
	vertexArray[count++] = Vertex(glm::vec3(0, y, 0), glm::vec2(0.5,0.5), glm::vec3(0, 1, 0));//se modifico esta linea para agregar texturas
	//bottom cap
	y = -y;
